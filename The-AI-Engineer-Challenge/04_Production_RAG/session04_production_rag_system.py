...
openai_client = None
# Lazy initialization of OpenAI client is a technique where the client is initialized only when it is needed, rather than when the class is instantiated. This is useful when the client is not needed immediately, and can help improve performance and reduce memory usage.
...
rag_system = LazyRAGSystem(db_manager, get_openai_client)   # LazyRAGSystem is a class that inherits from ProductionRAG
